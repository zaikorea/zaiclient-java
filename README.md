...
```css
dependencies {
  implementation 'com.github.zaikorea:zaiclient-java:v0.1.1'
}
```
...
- `PurchaseEvent` : 고객이 제품을 구매하거나, 영상을 전부 시청하는 등 어떠한 아이템을 완전히 소비했을 때 사용 가능한 이벤트입니다. 추가 인풋으로 `int price`가 들어가며, 제품의 구매 당시 가격을 기록하면 됩니다.
